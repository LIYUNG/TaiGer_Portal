...
const ProcessMessages = async (student, messages) => {
  ...
  if (messages.length > 1) {
    ...
    for (const msg of messages) {
      if (msg === undefined && msg.user_id?.role === Role.Student && msg.ignore_message !== true) {
        msg1 = msg;
    ...
    for (const student of students) {
      try {
        for (const generaldocs_thread of student.generaldocs_threads) {
          const messages = generaldocs_thread.doc_thread_id.messages;

          await ProcessThread(messages);
        }
      } catch (e) {
