      ...
      (resp) => {
        ...
        if (success) {
          setCommunicationsState((prevState) => ({
            ...prevState,
            success,
            upperThread: [...data, ...communicationsState.upperThread],
            isLoaded: true,
            student,
            pageNumber: communicationsState.pageNumber + 1,
            uppderaccordionKeys: [
              ...new Array(communicationsState.uppderaccordionKeys.length)
                .fill()
                .map((x, i) => i),
              ...new Array(data.length)
                .fill()
                .map((x, i) =>
                  communicationsState.uppderaccordionKeys !== -1
                    ? i + communicationsState.uppderaccordionKeys.length
                    : -1
                )
            ],
